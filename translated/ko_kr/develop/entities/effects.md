...
Mob effects, also known as status effects or simply effects, are a condition that can affect an entity. 이는 자연적으로 좋거나, 나쁘거나, 중립적일 수 있습니다. 기본 게임은 이러한 효과를 음식, 물약 등 다양한 방법으로 적용합니다.
...
`MOB_EFFECT` registry. 이는 모드 초기화 단계에서 완료되어야 합니다.

@[code lang=java transcludeWith=:::1](@/reference/latest/src/main/java/com/example/docs/effect/ExampleModEffects.java)

### 텍스쳐 {#texture}

The mob effect icon is a 18x18 PNG which will appear in the player's inventory screen. 사용자 정의 아이콘을 다음 폴더에 넣어 적용할 수 있습니다:
