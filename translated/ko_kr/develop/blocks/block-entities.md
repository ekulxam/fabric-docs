---
title: 블록 엔티티
description: 나만의 맞춤 블록을 위한 블록 엔티티를 만드는 법을 알아보세요.
authors:
  - natri0
---

블록 엔티티(Block Entities)는 블록 상태에 포함되지 않는 추가 데이터를 저장하는 방법입니다. 예를 들어, 인벤토리 내용물, 사용자 지정 이름 등이 해당됩니다.
Minecraft는 상자, 화로, 커맨드 블록 등과 같은 블록에 블록 엔티티를 사용합니다.

예시로, 마우스 오른쪽 클릭을 얼마나 했는지 확인하는 블록을 만들 겁니다.

## 블록 엔티티 만들기 {#creating-the-block-entity}

Minecraft가 새로운 블록 엔티티를 인식하고 로드하도록 하려면, 블록 엔티티 유형을 만들어야 합니다. 이는 `BlockEntity` 클래스를 확장하고 새로운 `ModBlockEntities` 클래스에 등록함으로써 이루어집니다.

@[code transcludeWith=:::1](@/reference/latest/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)

`BlockEntity`를 등록하면 위에서 사용한 `COUNTER_BLOCK_ENTITY`와 같은 `BlockEntityType`이 생성됩니다:
