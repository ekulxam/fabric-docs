...
Next, we have to append the property to the blockstate manager in the `createBlockStateDefinition` method. Dovrai fare override del metodo per accedere al costruttore:
...
Questo esempio invertirà la proprietà booleana `activated` quando il giocatore interagisce con il blocco. We can override the `useWithoutItem` method for this:
