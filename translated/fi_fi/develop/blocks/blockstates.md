---
title: Kuutioiden tilat
description: Opi miksi kuutioiden tilat ovat erinomainen tapa lisätä visuaalista toiminnallisuutta kuutioihisi.
authors:
  - IMB11
---

Kuution tila on Minecraftin maailmassa yksittäiseen palikkaan liitetty datan palanen, joka sisältää tietoa palikasta ominaisuuksien muodossa - esimerkkejä ominaisuuksista, joita moditon minecraft tallentaa kuution tiloihin:

- Kääntö: Käytetään lähinnä tukeille ja muille luonnollisille kuutioille.
- Aktivoitu: Käytetään paljon punakivilaitteissa ja kuutioissa kuten uuni ja savustin.
- Ikä: Käytetään viljelyksissä, kasveissa, taimissa, levässä jne.

Luultavasti ymmärrät miksi ne ovat hyödyllisiä - ne välttävät tarpeen varastoida NBT dataa kuutioyksikköön - pienentäen maailman kokoa ja välttäen TPS-ongelmia!

Blockstate definitions are found in the `assets/example-mod/blockstates` folder.

## Esimerkki: Pilarikuutio {#pillar-block}

<!-- Note: This example could be used for a custom recipe types guide, a condensor machine block with a custom "Condensing" recipe? -->

Minecraftilla on jo joitain luokkia, jotka mahdollistavat sinun luoda tietyn tyyppisiä kuutioita nopeasti - tämä esimerkki näyttää, kuinka luodaan kuutio jolla on `akseli` ominaisuus luomalla "kondensoitu tammitukki" kuutio.

The vanilla `RotatedPillarBlock` class allows the block to be placed in the X, Y or Z axis.

@[code transcludeWith=:::3](@/reference/latest/src/main/java/com/example/docs/block/ModBlocks.java)

Pilarikuutioilla on kaksi tekstuuria, päällys ja kylki - ne käyttävät `block/cube_column` mallia.
