---
title: Kodeky
description: Srozumitelný návod na používání kodekového systému Mojangu pro serializaci a deserializaci objektů.
authors:
  - enjarai
  - Syst3ms
---

Kodeky jsou systém umožňující jednoduchou serializaci Java objektů a jsou součástí knihovny DataFixerUpper (DFU), která je součástí Minecraftu. In a modding context they can be used as an alternative
...
Kodeky jsou používány spolu s další API z DFU, `DynamicOps` (dynamické operace). A codec defines the structure of an object, while dynamic ops are used to define a format to be serialized to and from, such as JSON or NBT. This means any codec can be used with any dynamic ops, and vice versa, allowing for great flexibility.

## Používání kodeků {#using-codecs}

### Serializace a deserializace {#serializing-and-deserializing}

Kodeky se běžně používají k serializaci a deserializaci objektů z a do určitého formátu.

Jelikož několik základních tříd již má definované kodeky, můžeme je použít jako příklad. Mojang has also provided us with two dynamic ops classes by default, `JsonOps` and `NbtOps`, which tend to cover most use cases.
...
| `Codec<A>#comapFlatMap` | Ne                   | Yes                  |
| `Codec<A>#flatComapMap` | Yes                  | Ne                   |
| `Codec<A>#flatXMap`     | Ne                   | Ne                   |
