...
description: Créer des commandes avec des paramètres et des actions complexes.
authors:
  - Atakku
  - dicedpixels
  - haykam821
...
La création de commandes permet à un développeur de mods d'ajouter des fonctionnalités qui peuvent être utilisées par le biais d'une commande. Ce tutoriel va vous apprendre comment enregistrer des commandes et la structure générale des commandes de Brigadier.
...
`com.mojang.brigadier.Command` est une interface fonctionnelle qui exécute un code spécifique et lève une `CommandSyntaxException` dans certains cas. It has a generic type `S`, which defines the type of the _command source_.
The command
...
L'entier ne peut pas être considéré comme partie du résultat de la commande. Les values qui sont égaux ou moins que zéro veut typiquement dire une commande a raté et n'a fait rien. Les valeurs positives veulent signifie la commande avait effectué quelque choisi avec du succès. Brigader fournir une constante pour indicer le succès ; `Commande#SINGLE_SUCCESS`.
...
`CommandSyntaxException` est généralement levée pour indiquer des erreurs de syntaxe dans les commandes ou les arguments. Vous pouvez aussi implémenter votre propre exception.
