---
title: Créer votre premier Bloc
description: Apprenez à créer votre premier bloc personnalisé dans Minecraft.
authors:
  - CelDaemon
  - Earthcomputer
  - IMB11
...
Les blocs sont les éléments de base de Minecraft - comme tout le reste dans Minecraft, ils sont stockés dans des registres (`registries`).

## Définir votre classe de Blocs {#preparing-your-blocks-class}
...
Vous devez placer cette méthode dans une classe appelée `ModBlocks` (ou le nom que vous souhaitez).

Mojang fait quelque chose de très similaire avec les blocs de la version vanilla ; vous pouvez vous référer à la classe `Blocks` pour voir comment ils le font.

@[code transcludeWith=:::1](@/reference/latest/src/main/java/com/example/docs/block/ModBlocks.java)

Comme pour les objets, vous devez vous assurer que la classe est chargée afin que tous les champs statiques contenant vos instances de bloc soient initialisés.
