...
ブロックはMinecraftの建築ブロックです（文字通りです）。Minecraftの他の要素と同じく、ブロックもレジストリに登録されます。

## ブロック登録用クラスの作成 {#preparing-your-blocks-class}

まず、ブロックとブロックアイテム(この2つは別物)をレジストリに登録するためのメソッドを、ブロック登録用クラスに作成します。[初めてのアイテム作成](../items/first-item) を終えていれば、アイテムの登録ととても似たプロセスであることがわかると思います。

ブロック登録用クラスのクラス名は、この例で用いる`ModBlocks`でも、任意の好きな名前でも構いません。

Mojang公式でも、既存のブロックの登録処理で非常によく似た処理をしています。`net.minecraft.item`パッケージの`Blocks`クラスを参照して、どのような処理か確認してみてください。

@[code transcludeWith=:::1](@/reference/latest/src/main/java/com/example/docs/block/ModBlocks.java)

アイテム登録と同様に、ブロックを含む全てのstatic フィールドを初期化するため、このクラスを明示的にロードする必要があります。
