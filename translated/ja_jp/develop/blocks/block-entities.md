---
title: ブロックエンティティ
description: カスタムブロック用のブロックエンティティを作成する方法を学びましょう。
authors:
  - natri0
---

ブロックエンティティは、ブロックのステートの一部ではない追加のデータをブロックに保存する方法です。例えば、インベントリの内容やカスタム名などです。
Minecraftでは、チェストやかまど、コマンドブロックなどにブロックエンティティが使われています。

例として、右クリックされた回数をカウントするブロックを作成します。

## ブロックエンティティの作成 {#creating-the-block-entity}

Minecraftが新しいブロックエンティティを認識して読み込むようにするためには、ブロックエンティティの種類を作成する必要があります。 これは `BlockEntity` クラスを拡張し、新しい `ModBlockEntities` クラスに登録することで行います。

@[code transcludeWith=:::1](@/reference/latest/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)

`BlockEntity` を登録すると、上記で使用した `COUNTER_BLOCK_ENTITY` のような `BlockEntityType` が得られます。
