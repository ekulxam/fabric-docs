...
このページでは、アイテムに関する重要なコンセプトと、アイテムの登録、テクスチャ、モデル、(翻訳後の)名前の付け方などについて解説します。

Minecraftでは、アイテムを含むすべての要素が「レジストリ(Registry)」と呼ばれるシステムに登録・保存されています(初めて聞く方もいるかもしれません)。

## アイテム登録用クラスの作成 {#preparing-your-items-class}
...
このメソッドは、引数で指定した識別子を用いてアイテムを作成し、そのアイテムをゲーム内のアイテム用レジストリに登録します。

また、このメソッドを定義するクラスは、`ModItems`でも、任意の好きな名前でも構いません。

参考として、Mojang公式のアイテム追加処理でも同様の仕組みで行われています！ net.minecraft.itemパッケージの`Items`クラスを確認してみてください。
...
## アイテムの登録 {#registering-an-item}

上で作成したクラスのregisterメソッドを使って、実際にアイテムを登録します。

The register method takes in an instance of the `Item.Properties` class as a parameter. `Item.Settings`クラスは、ビルドに関する様々なメソッドを用いて、アイテムの設定を定義することができます。

::: tip

If you want to change your item's stack size, you can use the `stacksTo` method in the `Item.Properties` class.

ただし、個別にダメージが蓄積されるアイテムの場合、アイテム毎にダメージ量が異なるため、スタックサイズは常に 1 のままとなり、設定を変更しても効果はありません。
