...
Creative Tabs, also known as Item Groups, are the tabs in the creative inventory that store items. You can create your own creative tab to store your items in a separate tab. これは、多くのアイテムを追加するMODを作る際にとても便利です。アイテムを一か所にまとめて整理できるので、プレイヤーも探しているアイテムに簡単にアクセスできるようになります。
...
You should see a new tab is now in the creative inventory menu. ただし、まだ言語設定別の名前を持っていません。[初めてのアイテム作成](./first-item) と同じように、言語ファイルに翻訳キーペアを追加する必要があります。

![Creative Tab without translation in creative menu](/assets/develop/items/itemgroups_0.png)

## 翻訳キーペアの追加 {#adding-a-translation-key}
