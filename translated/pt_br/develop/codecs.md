---
title: Codecs
description: Um guia para entender e usar o sistema de codificação da Mojang para serializar e desserializar objetos.
authors:
  - enjarai
  - Syst3ms
---

Codec é um sistema serializando objetos do Java, e inclui DataFixerUpper (DFU) da biblioteca da Mojang, No qual é incluindo com o Minecraft. In a modding context they can be used as an alternative
...
Codecs são usados em conjunto com outra API do DFU, `DynamicOps` (operações dinâmicas). A codec defines the structure of an object, while dynamic ops are used to define a format to be serialized to and from, such as JSON or NBT. This means any codec can be used with any dynamic ops, and vice versa, allowing for great flexibility.

## Usando Codecs {#using-codecs}

### Serializando e desserializando {#serializing-and-deserializing}

O uso básico de um codec é serializar e desserializar objetos de/para uma específica forma.

Visto que algumas classes vanilla já tem codecs definidos, vamos usar essas como exemplo. Mojang has also provided us with two dynamic ops classes by default, `JsonOps` and `NbtOps`, which tend to cover most use cases.
...
### Construídas em Codecs {#built-in-codecs}
...
A API Codec em si também contém alguns tipos primitivos para codecs, como `Codec.INT` e `Codec.STRING`. These are available as statics on the `Codec` class, and are usually used as the base for more complex codecs, as explained below.

## Construindo Codecs {#building-codecs}

Agora que vimos como usar codecs, vamos ver como podemos escrever o nosso. Suppose we have the following class, and we want to deserialize instances of it from JSON files:
...
Podemos fazer um codec para essa classe usando vários mini codecs para criar um maior. In this case, we'll need one for each field:

- o `Codec<Integer>`
- o `Codec<Item>`
- o `Codec<List<BlockPos>>`

Podemos conseguir o primeiro dos Codecs primitivos mencionado acima na classe `Codec`, especificamente `Codec.INT`. While the second one can be obtained from the `BuiltInRegistries.ITEM` registry, which has a `byNameCodec()` method that returns a `Codec<Item>`. Não temos um Codec padrão para `List<BlockPos>`, porém podemos fazer um do `BlockPos.CODEC`.

### Listas {#lists}

`Codec#listOf` pode ser usado para criar uma lista de versão de qualquer codec:

```java
Codec<List<BlockPos>> listCodec = BlockPos.CODEC.listOf();
```

Pode se notar que os codecs criados nessa maneira podem ser desserializados para um `ImmutableList`. If you need a mutable list instead, you can make use of [xmap](#mutually-convertible-types) to convert to one during
deserialization.

### Emergindo Codecs por classes tipo Record {#merging-codecs-for-record-like-classes}
...
| Método                  | A -> B é sempre válido? | B -> A é sempre válido? |
| ----------------------- | ----------------------- | ----------------------- |
| `Codec<A>#xmap`         | Yes                     | Yes                     |
| `Codec<A>#comapFlatMap` | Não                     | Yes                     |
| `Codec<A>#flatComapMap` | Yes                     | Não                     |
| `Codec<A>#flatXMap`     | Não                     | Não                     |
