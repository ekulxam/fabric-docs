---
title: Criando Seu Primeiro Item
description: Aprender a como registrar um item simples e como texturizá-lo, modelá-lo e nomeá-lo.
authors:
  - dicedpixels
  - Earthcomputer
  - IMB11
...
Esta página irá apresentar alguns conceitos-chave relacionados a itens e como você pode registrá-los, texturizá-los, modelá-los e nomeá-los.

Caso você não saiba, tudo no Minecraft é armazenado em registries, e itens não são uma exceção.

## Preparando a Sua Classe de Itens {#preparing-your-items-class}
...
Este método irá criar um item com o identificador fornecido e registrá-lo no registro de itens do jogo.

Você pode colocar este método em uma classe chamada `ModItems` (ou qualquer nome que preferir para a classe).

A Mojang faz o mesmo com itens deles! Dê uma olhada na classe `Items` para se inspirar.
...
## Registrando um Item {#registering-an-item}

Agora você pode registrar um item usando o método.

The register method takes in an instance of the `Item.Properties` class as a parameter. Essa classe permite que você configure as propriedades do item através de vários métodos de builder.

::: tip

If you want to change your item's stack size, you can use the `stacksTo` method in the `Item.Properties` class.

Isso não funcionará se você tiver marcado o item como damageable, pois o tamanho da pilha é sempre 1 para itens danificáveis, para evitar exploits de duplicação.
