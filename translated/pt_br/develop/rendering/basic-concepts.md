---
title: Conceitos básicos de renderização
description: Aprenda sobre os conceitos básicos de renderização usando o motor gráfico do Minecraft.
authors:
  - "0x3C50"
  - IMB11
  - MildestToucan
...
Quaterniões são uma formade representar rotação no espaço. They are used to rotate the top matrix on the `PoseStack` via the `rotateAround(quaternionfc, x, y, z)` method.
