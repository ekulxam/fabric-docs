---
title: Creando tu primer item
description: Aprende a registrar un item simple y a darle textura, modelo y nombre.
authors:
  - dicedpixels
  - Earthcomputer
  - IMB11
...
Esta página te introducirá en algunos conceptos clave relacionados con los items, y cómo puedes registrarlos, texturizarlos, modelarlos y nombrarlos.

Por si no lo sabes, en Minecraft todo se almacena en registros, y los items no son una excepción.

## Preparando tu clase ModItems {#preparing-your-items-class}
...
Este método creará un item con el identificador proporcionado y lo registrará en el registro de items del juego.

Puedes poner este método en una clase llamada `ModItems` (o como quieras llamar a la clase).

Mojang también hace esto con sus items! Echa un vistazo a la clase `Items` para inspirarte.
...
## Registrando un item {#registering-an-item}

Ahora puedes registrar un nuevo item utilizando este metodo.

The register method takes in an instance of the `Item.Properties` class as a parameter. Esta clase permite configurar las propiedades del item a través de varios métodos.

::: tip

If you want to change your item's stack size, you can use the `stacksTo` method in the `Item.Properties` class.

Esto no funcionará si has marcado el objeto como dañable, ya que el tamaño del stack es siempre 1 para los objetos dañables para evitar que se dupliquen.
