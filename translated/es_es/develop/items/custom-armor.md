---
title: Armaduras Pesonalizadas
description: Aprende a como crear tus propias armaduras.
authors:
  - ""
---

Las armaduras proporcionan al jugador una mayor defensa contra los ataques de mobs y otros jugadores.

## Creando una clase de materiales de armadura {#creating-an-armor-materials-class}

Técnicamente, no necesitas una clase dedicada para tu material de armadura, pero sigue siendo una buena práctica debido a la cantidad de campos estáticos que necesitarás.

For this example, we'll create a `GuiditeArmorMaterial` class to store our static fields.

### Durabilidad Base {#base-durability}
...
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `durabilidad`         | The base durability of all armor pieces, this is used when calculating the total durability of each individual armor piece that use this material. This should be the base durability constant you created earlier. |
