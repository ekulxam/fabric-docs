---
title: Блок обекти
description: Научете как да създавате блокови обекти за вашите персонализирани блокове.
authors:
  - natri0
---

Блоковите обекти са начин за съхраняване на допълнителни данни за блок, които не са част от състоянието на блока: съдържание на инвентара, персонализирано име на блока и т.н.
Minecraft използва блокови обекти за блокове като сандъци, печки и командни блокове.

На пример ще създадем блок, който брои колко пъти е натиснат с десен бутон.

## Създаване на блоковият обект{#creating-the-block-entity}

За да накараме Minecraft да разпознае и зареди новите блокови обекти, трябва да създадем тип обект. Това се прави чрез разширяване на класа `BlockEntity` и регистрирането му в нов клас `ModBlockEntities`.

@[code transcludeWith=:::1](@/reference/latest/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)

Регистрирането на `BlockEntity` дава `BlockEntityType`, подобен на `COUNTER_BLOCK_ENTITY`, който използвахме по-горе:
