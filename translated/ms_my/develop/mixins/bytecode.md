---
title: Bytecode Java
description: Ketahui tentang bytecode Java supaya anda boleh menulis mixin dengan berkesan.
authors:
  - Earthcomputer
  - its-miroma
---

Mixin beroperasi pada bytecode Java, jadi untuk memahaminya seseorang memerlukan pemahaman tentang asasnya.
...
## Nama dan Simbol {#names-and-symbols}

Banyak perkara dalam bytecode, seperti kelas, medan dan kaedah, masih dikenal pasti dengan nama (dan deskriptor untuk medan dan kaedah, nanti kita akan bincangkannya), sama seperti ia berada dalam kod sumber. Walau bagaimanapun, format sebenar nama-nama tersebut sedikit berbeza.

### Nama Kelas {#class-names}

Kelas biasanya dirujuk dengan _nama dalaman_ mereka, yang secara kasarnya bersamaan dengan nama kelas yang layak sepenuhnya (nama penuh termasuk pakej), di mana semua titik `.` digantikan dengan garis miring `/`. Contohnya, nama dalaman kelas `java.lang.Object` ialah `java/lang/Object`.

Kelas bersarang menggunakan simbol `$` untuk memisahkan namanya daripada kelas yang dilampirkan. Contohnya, diberikan:
...
... nama dalaman `Bar` ialah `pkg/Foo$Bar`.

Kelas tanpa nama menggunakan nombor dan bukannya nama. Contohnya, jika terdapat dua kelas tanpa nama dalam kelas `Foo` daripada blok kod di atas, nama dalamannya masing-masing ialah `pkg/Foo$1` dan `pkg/Foo$2`.

Kelas tempatan (kelas yang ditakrifkan dalam kaedah) mempunyai nombor diikuti dengan namanya. Contohnya, nama kelas setempat mungkin kelihatan seperti `pkg/Foo$1Local`.

### Deskriptor Jenis {#type-descriptors}

Apabila bytecode perlu merujuk kepada jenis atau tatasusunan primitif, _deskriptor jenis_ akan digunakan. Berikut ialah jadual jenis data dan deskriptor jenis masing-masing:
...
| `short`     | `S`                                                                        |
| `void`      | `V`                                                                        |
| Tatasusunan | `[` + jenis elemen: `int[]` -> `[I`                        |
| Objek       | `L` + nama dalaman + `;`: `String` -> `Ljava/lang/String;` |

### Deskriptor Medan dan Kaedah {#field-and-method-descriptors}

Dalam bytecode, medan dan kaedah dikenal pasti dengan menggabungkan nama dan _deskriptor_ mereka. Untuk medan, itulah deskriptor jenis datanya.

Sebaliknya, kaedah memperolehnya dengan menggabungkan jenis parameter dan jenis pemulangan. Contohnya, kaedah berikut:
...
... mempunyai deskriptor `(IILjava/lang/String;I)V`.

Deskriptor untuk jenis parameter digabungkan secara langsung bersama, tanpa pemisah. Dalam kes ini, terdapat `I` untuk `int` sebanyak dua kali (untuk kedua `x` dan `y`), kemudian `Ljava/lang/String;` untuk `String` (`text`), satu lagi `I` untuk `int` terakhir (`color`).

### Pembina dan Pemula Statik {#constructors-and-static-initializers}

Pada peringkat bytecode, pembina hanyalah satu lagi kaedah: perbezaan terperinci antara kedua-duanya berada di luar skop gambaran keseluruhan ini.

Nama kaedah pembina ialah `<init>` (dengan kurungan bersudut `<>`), dan jenis pulangan dalam deskriptornya ialah `V` (`void`). Semua permulaan medan bukan statik, selepas penyusunan, akan ditemui di dalam kaedah `<init>`.

Sebaliknya, pemula statik (blok `static {}` dalam kod sumber, serta pemula medan statik dengan beberapa pengecualian) juga hanyalah satu lagi kaedah yang dijalankan apabila kelas dimuatkan: namanya ialah `<clinit>`, dan deskriptornya ialah `()V`.

## Pemboleh Ubah Tempatan {#local-variables}

Dalam kod sumber, pemboleh ubah tempatan dikenal pasti dengan namanya. Dalam bytecode, ia sebaliknya dikenal pasti dengan nombor, atau indeks ke dalam Jadual Pemboleh Ubah Tempatan (LVT). Parameter kaedah disertakan dalam LVT, begitu juga objek `this` dalam kaedah bukan statik.

Pertimbangkan kaedah berikut sebagai contoh:
...
Dalam bytecode itu, `this` mendapat indeks 0, `offset` mendapat indeks 1 dan `result` mendapat indeks 2.

Kaedah statik tidak mempunyai `this` dalam LVT, jadi parameter pertama kaedah statik mendapat indeks 0 secara langsung.

Long dan double mengambil 2 indeks dalam LVT. Contohnya, dalam kaedah statik berikut:
...
... parameter `x` mendapat indeks 0, parameter `y` mendapat indeks 2, dan parameter `z` mendapat indeks 4.
