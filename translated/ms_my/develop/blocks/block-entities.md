---
title: Entiti Blok
description: Ketahui cara membuat entiti blok untuk blok tersuai anda.
authors:
  - natri0
---

Entiti blok ialah satu cara untuk menyimpan data tambahan untuk sebuah blok, yang bukan sebahagian daripada keadaan blok: kandungan inventori, nama tersuai dan sebagainya.
Minecraft menggunakan entiti blok untuk blok seperti peti, relau dan blok perintah.

Sebagai contoh, kami akan membuat blok yang mengira berapa kali ia telah diklik kanan.

## Mencipta Entiti Blok {#creating-the-block-entity}

Untuk membuatkan Minecraft mengenali dan memuatkan entiti blok baharu, kita perlu mencipta satu jenis entiti blok. Ini dilakukan dengan melanjutkan kelas `BlockEntity` dan mendaftarkannya dalam kelas `ModBlockEntities` baharu.

@[code transcludeWith=:::1](@/reference/latest/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)

Pendaftaran `BlockEntity` akan menghasilkan `BlockEntityType` seperti `COUNTER_BLOCK_ENTITY` yang telah kami gunakan di atas:
