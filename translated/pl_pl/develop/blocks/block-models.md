---
title: Modele bloków
description: Przewodnik po pisaniu i rozumieniu modelów bloków.
authors:
  - Fellteros
  - its-miroma
---
...
Ta strona przeprowadzi Cię przez proces tworzenia własnych modeli do bloków i pozwoli Ci zrozumieć wszystkie ich opcje i możliwości.

## Czym są modele bloków? {#what-are-block-models}

Modele bloków stanowią w zasadzie definicję wyglądu i elementów wizualnych bloku. Określają teksturę, przesunięcie modelu, obrót skalę i inne atrybuty.

Modele są przechowywane jako pliki JSON w folderze `resources`.

## Struktura pliku {#file-structure}

Każdy plik modelu bloku ma zdefiniowaną strukturę, której należy przestrzegać. It starts with empty curly brackets, which represent the **root tag** of the model. Here's a brief scheme of how block models are structured:
