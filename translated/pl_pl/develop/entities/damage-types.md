---
title: Damage Types
description: Naucz się dodawać własne typy obrażeń.
authors:
  - dicedpixels
  - hiisuuii
  - MattiDragon
...
Typy obrażeń definiują typy obrażeń, które byty mogą przyjąć. Od wersji Minecraft 1.19.4, tworzenie nowych typów obrażeń stało się oparte na danych, co znaczy, że będą tworzone, używając plików JSON.
...
To miało następującą strukturę:
...
### Używany typ obrażeń {#using-damage-types}

By zademonstrować niestandardowy typ obrażeń, użyjemy niestandardowego bloku nazwanego _Tater Block_. Let's make is so that
...
Kompletna implementacja bloku:

@[code lang=java transcludeWith=:::1](@/reference/latest/src/main/java/com/example/docs/damage/TaterBlock.java)

Teraz, kiedykolwiek żyjąca istota nadepnie na nasz niestandardowy blok, otrzyma 5 obrażeń (2,5 serca), używając naszego niestandardowego typu obrażeń.

### Niestandardowa wiadomość śmierci {#custom-death-message}
...
```json
{
  "death.attack.tater": "%1$s umarł od obrażeń Tater!"
}
```
...
### Znaczniki typów obrażeń {#damage-type-tags}

Some damage types can bypass armor, bypass status effects, and such. Tags are used to control these kinds of properties
of damage types.

Możesz znaleźć istniejące znaczniki typów obrażeń w `data/minecraft/tags/damage_type`.
