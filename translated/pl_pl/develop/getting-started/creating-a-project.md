...
Code examples given on this site use [Mojang's official names](../migrating-mappings/#mappings). Jeśli Twój mod nie korzysta z tych samych mapowań, w których napisano tę dokumentację, będziesz musiał przekonwertować przykłady, korzystając z witryn takich jak [mappings.dev](https://mappings.dev/) lub [Linkie](https://linkie.shedaniel.dev/mappings?namespace=yarn&translateMode=ns&translateAs=mojang_raw&search=).
