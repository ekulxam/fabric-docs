...
description: Руководство по настройке генерации достижений через Data Generator.
authors:
  - CelDaemon
  - MattiDragon
  - skycatminepokie
...
Достижение сделано из нескольких разных компонентов. Along with the requirements, called "criterion," it may have:

- `DisplayInfo` которая говорит игре как показывать достижение игрокам,
...
- `AdvancementRewards`, что игрок получает за выполнение достижения.
...
Вот простое достижение за получение блока земли:
...
## Ещё один пример {#one-more-example}

Чтобы финально заучить это, давайте создадим ещё одно достижение. We'll practice adding rewards, using multiple criterion, and assigning parents:
...
Сначала нам надо придумать новую механику. Давайте говорить игроку какой предмет он использовал каждый раз ломая блок.
...
Учтите то что этот код очень плох. The `HashMap` is not stored anywhere persistent, so it will be reset every time the game is restarted. It's just to show off `Criterion`s. Начните игру и попробуйте!
...
Вау, это много! Давайте во всём разберёмся.
