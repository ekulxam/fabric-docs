---
title: Генерация модели предмета
description: Руководство по созданию моделей предметов с помощью datagen.
authors:
  - CelDaemon
  - Fellteros
  - skycatminepokie
...
Убедитесь что вы завершили [установку datagen](./setup) и создали ваш [первый предмет](../items/first-item).

:::

Для каждой модели которую мы хотим сгенерировать, потребуется создать два отдельных JSON файла:

1. **Модель предмета**, которая определяет текстуры, вращение и общий вид предмета. Он находится в каталоге `generated/assets/example-mod/models/item`.
2. **Клиентский предмет**, который определяет, какая модель должна использоваться, базируясь на основе различных критериев, таких как компоненты, взаимодействия и т. д. Он находится в каталоге `generated/assets/example-mod/items`.
...
Затем создайте конструктор соответствующий `super`.
...
### Вызов пользовательского метода {#custom-call}
...
Для получения дополнительной информации вы можете просмотреть примеры тестов в [Fabric API](https://github.com/FabricMC/fabric/blob/1.21.11/fabric-data-generation-api-v1/src/) и [Example Mod](https://github.com/FabricMC/fabric-docs/tree/main/reference) в этой документации.
