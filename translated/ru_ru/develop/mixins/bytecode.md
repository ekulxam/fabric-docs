...
## Названия и символы {#names-and-symbols}

Во многих вещах в байт-коде, таких как классы, поля и методы, по-прежнему используются названия для идентификации (а для полей и методов — ещё и дескрипторы, об этом позже), как и в исходном коде. Однако точный формат этих названий немного отличается.

### Названия классов {#class-names}
...
Дескрипторы для параметров объединяются напрямую, без разделителей. В данном случае, `I` использована для `int` дважды (для `x` и `y`), далее `Ljava/lang/String;` для `String` (`text`), а далее ещё одна `I` для последнего `int` (`color`).
