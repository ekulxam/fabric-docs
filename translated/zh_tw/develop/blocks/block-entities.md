---
title: 方塊實體
description: 學習如何為自定義方塊建立方塊實體。
authors:
  - natri0
---

方塊實體是為方塊添加資料的方法，且不像是方塊狀態如：內容物、自定義名稱等。
Minecraft 在箱子、熔爐和指令方塊等地方使用方塊實體。

作為例子，我們將建立能夠計算右鍵點擊次數的方塊。

## 建立方塊實體 {#creating-the-block-entity}

要讓 Minecraft 辨別並載入新的方塊實體，我們需要建立方塊實體類型。 這可以通過擴展 `BlockEntity` 類別並註冊於新的 `ModBlockEntities` 類別。

@[code transcludeWith=:::1](@/reference/latest/src/main/java/com/example/docs/block/entity/custom/CounterBlockEntity.java)

註冊一個 `BlockEntity` 會產生一個 `BlockEntityType`，像是上面的 `COUNTER_BLOCK_ENTITY`：
