...
Minecraft sandıklar, fırınlar ve komut blokları gibi bloklar için blok oluşumları kullanır.

Örneğin, Kaç defa sağ tıklandığını sayan bir blok yaratıcaz.

## Blok Oluşumu Yaratılıyor {#creating-the-block-entity}

Minecraft'ın yeni Blok oluşumlarını tanıması ve yüklemesi için Blok oluşumu tipi yaratmalıyız. This is done by extending the `BlockEntity` class and registering it in a new `ModBlockEntities` class.
